...
void print_u8_binary(u8 value) {
    char binary_string[9];  // 8 bits + null terminator
    int i;

    for (i = 0; i < 8; i++) {
        binary_string[7 - i] = (value & (1 << i)) ? '1' : '0';  // Set '1' or '0' based on the bit
    }
    binary_string[8] = '\0';  // Null-terminate the string

    pr_info("entry->addr.id %s\n", binary_string);
}

void print_bitmap_binary(const unsigned long *bitmap, int bits);
...
void print_bitmap_binary(const unsigned long *bitmap, int bits)
{
	...
	binary_string = kmalloc(
		bits + 1, GFP_KERNEL); // Allocate memory for the binary string
	if (!binary_string) {
		pr_err("Failed to allocate memory for binary string\n");
		return;
	}

	for (i = 0; i < bits; i++) {
		binary_string[bits - 1 - i] =
			test_bit(i, bitmap) ?
				'1' :
				'0'; // Convert each bit to '0' or '1'
	}
	binary_string[bits] = '\0'; // Null-terminate the string

	pr_info("msk->pm.id_avail_bitmap %s\n", binary_string);

	kfree(binary_string); // Free the allocated memory
}
		     ...
		     const struct mptcp_sock *msk)
{
	...
	pr_info("Selecting local address, available IDs: %*pb\n",MPTCP_PM_MAX_ADDR_ID + 1, msk->pm.id_avail_bitmap);
	rcu_read_lock();
	list_for_each_entry_rcu(entry, &pernet->local_addr_list, list) {
		pr_info("Checking address: id %d, addr %pI4, flags %x, available %d\n",entry->addr.id, &entry->addr.addr.s_addr, entry->flags, test_bit(entry->addr.id, msk->pm.id_avail_bitmap));
		
		pr_info("local_addr_used %d: addr %pI4: select_local_address() is running\n", msk->pm.local_addr_used, &entry->addr.addr.s_addr);

		if (!(entry->flags & MPTCP_PM_ADDR_FLAG_SUBFLOW)) {
			pr_info("local_addr_used %d: saddr %pI4: subflow is not a subflow\n", msk->pm.local_addr_used, &entry->addr.addr.s_addr);
			continue;
		}

		print_u8_binary(entry->addr.id);
		print_bitmap_binary(msk->pm.id_avail_bitmap, 8);
		if (!test_bit(entry->addr.id, msk->pm.id_avail_bitmap)) {
			pr_info("local_addr_used %d: saddr %pI4: address test failed\n", msk->pm.local_addr_used, &entry->addr.addr.s_addr);
			continue;
		}

		pr_info("local_addr_used %d: saddr %pI4: select_local_address() successful\n", msk->pm.local_addr_used, &entry->addr.addr.s_addr);
		ret = entry;
		break;
	}

	if (!ret)
		pr_info("local_addr_used %d: select_local_address() failed, not creating subflow\n", msk->pm.local_addr_used);
...
static void mptcp_pm_create_subflow_or_signal_addr(struct mptcp_sock *msk)
{
	...
	pr_info("check if should create a new subflow\n"
		"msk->pm.local_addr_used: %d, local_addr_max: %d\n"
		"msk->pm.subflows: %d, subflows_max: %d\n",
		msk->pm.local_addr_used, local_addr_max, msk->pm.subflows, subflows_max);
...
int mptcp_pm_nl_add_addr_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	*entry = addr;
	...
	pr_info("Added address: id %d, addr %pI4, flags %x\n",entry->addr.id, &entry->addr.addr.s_addr, entry->flags);
	return 0;
